...
import { allProviderPresets } from "@/config/claudeProviderPresets";

describe("Kimi For Coding Provider Preset", () => {
  const kimiForCoding = allProviderPresets.find(
...
describe("Codex Provider Preset", () => {
  const codex = allProviderPresets.find((p) => p.name === "Codex");
...
describe("AWS Bedrock Provider Presets", () => {
  const bedrockAksk = allProviderPresets.find(
  ...
  const bedrockApiKey = allProviderPresets.find(
